    ...
    render() {
        ...
        let HeadMenu = HeadData.map((item, index) => (
            <li className="headNavLi" key={index}>
                <Link to={
                    {
                        pathname: (item.aLink === '/product/list/' || item.aLink === '/solution/list/') ? item.cUrl + item.childrens
                            [0].name + "/" + item.childrens[0].id : item.aLink,
                        query: {
                            listId: item.id
                        },
                        // title:item.name,
                        search: (item.aLink === '/product/list/' || item.aLink === '/solution/list/') ? "id=" + item.childrens[0].id + "&type=" + item.childrens[0].type : "id=" + item.id,
                        state: {
                            listId: item.id
                        }
                    }
                } replace>{item.name}</Link>
                <div className="dropDwon">
                    <ul>
                        {
                            item.childrens.map((_item, _index) => (
                                <li className="dropDownLi" key={_index} data-id={_item.id} data-type={_item.url}
                    ...
                    <div className="Footer allwidth">
                        ...
                        <div className="footerBottom">
                            <div className="footerBottomCon">
                                ...
                                {/*鲁ICP备16007097号*/}
                                <a href={this.state.Copyright.url}>{this.state.Copyright.text}</a>
